...
describe('SearchBar', () => {
	...
	it('should submit highlighted result when Enter is pressed with listIndex > -1', async () => {
		const user = userEvent.setup();
		const $router = { push: vi.fn(), currentRoute: { fullPath: '/somewhere' } };
		const { getByPlaceholderText } = render(SearchBar, {
			global: {
				provide: { apollo: getMockApollo() },
				mocks: {
					lockScrollSmallOnly: vi.fn(),
					unlockScrollSmallOnly: vi.fn(),
					$kvTrackEvent: vi.fn(),
		...
		const input = getByPlaceholderText('Search all loans');
		await user.type(input, 'akr'); // More specific to match "Akron, OH"
		await user.type(input, '{arrowdown}'); // Highlight first result

		// Submit form
		await user.type(input, '{enter}');

		// Should navigate with the highlighted result's query
		expect($router.push).toHaveBeenCalledWith({
			path: '/lend/filter',
			query: { cityState: 'Akron,OH' }
		});
	});

	it('should handle string suggestion when no listIndex is selected', async () => {
		const user = userEvent.setup();
		...
		const input = getByPlaceholderText('Search all loans');
		await user.type(input, 'custom search');

		// Submit without arrow keys (listIndex = -1)
		await user.type(input, '{enter}');

		// Should track custom search event
		expect($kvTrackEvent).toHaveBeenCalledWith('search', 'click', 'custom-search-option', 'custom search');

		// Should navigate with queryString
		expect($router.push).toHaveBeenCalledWith({
			path: '/lend/filter',
			query: { queryString: 'custom search' }
		});
	});

	it('should navigate to external URL when suggestion has url property', async () => {
		const user = userEvent.setup();
		...
		const externalSuggestions = [
			{ group: 'Gifts', label: 'Kiva Cards', url: 'https://www.kiva.org/gifts/kiva-cards' },
		];

		const apollo = getMockApollo();
		apollo.query.mockReturnValue(Promise.resolve({
			data: { lend: { loanSearchSuggestions: externalSuggestions } }
		}));
		...
		const { getByPlaceholderText } = render(SearchBar, {
			global: {
				...
				mocks: {
					...
					unlockScrollSmallOnly: vi.fn(),
					$kvTrackEvent: vi.fn(),
					$router: { push: vi.fn(), currentRoute: { fullPath: '/somewhere' } },
				},
			},
		});

		const input = getByPlaceholderText('Search all loans');
		await user.type(input, 'cards');
		await user.type(input, '{arrowdown}'); // Highlight first result
		await user.type(input, '{enter}');

		// Should navigate to external URL
		expect(window.location).toBe('https://www.kiva.org/gifts/kiva-cards');

		// Restore window.location
		window.location = originalLocation;
	});

	it('should parse suggestion.query with = to extract key and value', async () => {
		const user = userEvent.setup();
		const $router = { push: vi.fn(), currentRoute: { fullPath: '/somewhere' } };
		const { getByPlaceholderText } = render(SearchBar, {
			global: {
		...
		await user.type(input, '{arrowdown}'); // Highlight "Alaska (AK)" result
		await user.type(input, '{enter}');

		// Should parse 'state=AK' to { state: 'AK' }
		// Note: 'state' is in excludedParams, so uses /lend not /lend/filter
		expect($router.push).toHaveBeenCalledWith({
			path: '/lend',
			query: { state: 'AK' }
		});
	...
	it('should use legacy /lend path when query has excluded params', async () => {
		const user = userEvent.setup();
		const $router = { push: vi.fn(), currentRoute: { fullPath: '/somewhere' } };

		// Import the actual module to spy on it
		const queryParamUtils = await import('#src/util/loanSearch/queryParamUtils');
		const hasExcludedQueryParamsSpy = vi.spyOn(queryParamUtils, 'hasExcludedQueryParams').mockReturnValue(true);
		...
		// Should use /lend instead of /lend/filter
		expect($router.push).toHaveBeenCalledWith({
			path: '/lend',
			query: { queryString: 'test' }
		});

		hasExcludedQueryParamsSpy.mockRestore();
	});

	it('should not push router when already on same path', async () => {
		const user = userEvent.setup();
		const $router = {
			push: vi.fn(),
			currentRoute: { fullPath: '/lend?state=AK' }
		};
		const { getByPlaceholderText } = render(SearchBar, {
			global: {
				provide: { apollo: getMockApollo() },
		...
		const input = getByPlaceholderText('Search all loans');
		await user.type(input, 'alaska');
		await user.type(input, '{arrowdown}'); // Highlight "Alaska (AK)" - state=AK
		await user.type(input, '{enter}');

		// Should NOT push because already on /lend?state=AK (state is in excludedParams, uses /lend not /lend/filter)
