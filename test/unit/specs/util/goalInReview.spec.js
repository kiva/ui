...
describe('goalRecapEntryPoint.js', () => {
	describe('current goal year', () => {
		...
		// The CTA and the pop-up arrive together, on the visit after the announcing one.
		it('holds the recap back on the visit that announces the completion', () => {
			expect(shouldShowRecapEntryPoint({
				...currentYearComplete,
				holdUntilNextVisit: true,
			})).toBe(false);
		});

		it('offers the recap on the next visit', () => {
			expect(shouldShowRecapEntryPoint({
				...currentYearComplete,
	...
	describe('next goal year', () => {
		...
		// The hold is about this year's win; a past goal's recap is already due.
		it('is not held back by the announcing visit for a past goal', () => {
			expect(shouldShowRecapEntryPoint({ ...nextYearComplete, holdUntilNextVisit: true })).toBe(true);
		});
	});
...
describe('goalInReviewCopy.js', () => {
	...
	describe('getImpactHabit', () => {
		...
		describe('read in a later year than the goal ran in', () => {
			it('looks back on the top percentile in the past tense', () => {
			...
			it('looks back on a Kiva champion\u2019s sessions in the past tense', () => {
			...
			it('looks back on a rising champion\u2019s sessions in the past tense', () => {
