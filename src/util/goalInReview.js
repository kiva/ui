...
/** Zoom used when there are no coordinates to size from (world-ish view). */
export const DEFAULT_MAP_ZOOM = 2;

// The map frames the cluster the goal reached rather than fitting every country:
// points further than (median radius x this multiplier) from the centre are treated
// as outliers and left out of the zoom calculation, so one far country doesn't force
// a whole-world zoom-out. Floored by MIN so a genuinely spread-out region isn't
// over-zoomed. Both are in degrees and tunable.
const MAP_CLUSTER_RADIUS_MULTIPLIER = 3;
const MAP_CLUSTER_MIN_RADIUS_DEG = 15;
...
// Normalise a longitude delta to [-180, 180] so spans/offsets stay wrap-aware.
...
// Median longitude, made wrap-safe by measuring each point's offset from the
// circular mean and taking the median of those offsets. Median (not mean) keeps a
// lone far country from dragging the centre off the cluster.
const getMedianLongitude = points => {
	const anchor = getMeanLongitude(points);
	return wrapDegrees(anchor + median(points.map(point => wrapDegrees(point.long - anchor))));
};

// Chebyshev distance (max of lat/long deltas) from a centre, wrap-aware on longitude.
const offsetFromCentre = (point, centre) => Math.max(
	Math.abs(point.lat - centre.lat),
	Math.abs(wrapDegrees(point.long - centre.long)),
);

// Mirrors KvMap's own computeMapZoom scale so our explicit zoom matches the
// component's built-in behaviour — just fed a cluster span instead of the full one.
const spanToZoom = maxSpan => {
	if (maxSpan < 1) return 6;
	if (maxSpan < 10) return 5;
	return Math.max(1, Math.min(Math.floor(Math.log2(360 / (maxSpan * 2.5))), 6));
};

/**
 * Centres the global reach slide map on the countries the goal reached — the country itself
 * when there is only one, otherwise the median position, which stays on the cluster even
 * when one country sits far from the rest.
 *
...
export function getCountriesMapCenter(countries = []) {
	...
	return {
		...
		long: getMedianLongitude(points),
	};
}

/**
 * Zoom level that frames the cluster of countries the goal reached. Points that sit far
 * outside the group are dropped before measuring the span, so a single distant country
 * lets the map zoom into the rest rather than zooming out to fit everything.
 *
 * @param {Array} countries Countries with `geocode.latitude` / `geocode.longitude`.
 * @returns {number} Zoom level between 1 and 6.
 */
export function getCountriesMapZoom(countries = []) {
	const points = getPoints(countries);
	if (!points.length) {
	...
	const centre = getCountriesMapCenter(countries);
	const radii = points.map(point => offsetFromCentre(point, centre));
	const threshold = Math.max(median(radii) * MAP_CLUSTER_RADIUS_MULTIPLIER, MAP_CLUSTER_MIN_RADIUS_DEG);
	const cluster = points.filter(point => offsetFromCentre(point, centre) <= threshold);

	const lats = cluster.map(point => point.lat);
	const longOffsets = cluster.map(point => wrapDegrees(point.long - centre.long));
	const latSpan = Math.max(...lats) - Math.min(...lats);
	const longSpan = Math.max(...longOffsets) - Math.min(...longOffsets);

	return spanToZoom(Math.max(latSpan, longSpan));
}
