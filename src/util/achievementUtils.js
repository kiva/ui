import { getRichTextUiSettingsData } from '#src/util/contentfulUtils';
...
export const buildAchievementSlides = ({
	...
	defaultBadges.forEach(badgeKey => {
		const achievementContent = badgesData.find(a => badgeKey === a.id);
		...
		const contentfulData = achievementContent.contentfulData.find(
			cData => cData.level === tier.level
		);
			...
			achievementSlides.push({
				...
				...(includeMilestoneDiff && {
					milestoneDiff: Math.max(
						tier.target
							- (achievementContent.achievementData?.totalProgressToAchievement ?? 0),
						0
					),
				}),
				target: tier.target,
				totalProgressToAchievement:
					achievementContent.achievementData?.totalProgressToAchievement,
