...
Prefetching runs over the whole component tree below the matched route, and a component is reached through the children the build step attaches to every compiled component, so it makes no difference whether a component is written with the Options API or with `<script setup>`. Only the Options API can author an `apollo` block, so a `<script setup>` component fetches through a composable instead — `useApolloQuery` with an exported `preFetchOperations`, covered under "Composable Data Fetching" below. See "How it works" there for what discovery does and does not reach.
...
The same plugin attaches each compiled component's imported children to its definition as `__childComponents`, and `getDeepComponents` walks that list alongside the `components` option, so the tree is discovered whether or not a component registers its children — a `<script setup>` component compiles its children into setup bindings and registers none. Children written as dynamic imports (the `defineAsyncComponent(() => import(...))` pattern) are attached as loader thunks and awaited during the walk, the same treatment registered async components already received, so importing a component lazily still costs a module load during prefetch.

Discovery reaches a child when the import is statically written and resolves to a `.vue` file in this repo. It does not reach a component re-exported through a `.js` module, a component imported from `@kiva/kv-components` or another prebuilt package, or a component chosen at runtime through `<component :is>`.
