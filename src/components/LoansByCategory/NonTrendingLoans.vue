<template>
	<div v-if="isLoggedIn" class="non-trending-holder" :class="{ 'non-trending-loans-loaded': isLoaded }">
		<div v-if="showNonTrendingLoans">
			<div v-if="isLoaded">
				<category-row
					class="loan-category-row"
					:key="nonTrendingCategory.id"
					:loan-channel="nonTrendingCategory"
					:items-in-basket="itemsInBasket"
					:row-number="-1"
					set-id="GROW-330-non-trending-loans"
...
export default {
	...
	data() {
		return {
			...
			showNonTrendingLoans: true,
			isLoaded: false,
			nonTrendingCategory: {
				id: 99,
				name: 'Spotlighted by Kiva',
				url: '/lend/spotlighted-by-kiva',
				loans: {
					values: [],
				},
			},
