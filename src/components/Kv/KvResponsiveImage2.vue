<template>
	...
	<!-- Send picture element and srcset to to get eager image fetching -->
...
export default {
	...
	computed: {
		...
		screenBreakpoints() {
			return _map(this.breakpoints.filter(breakpoint => breakpoint.size <= this.screenWidth), 'name');
		},
		...
		biggestImageInArray() {
			this.screenBreakpoints.forEach(breakpoint => {
				const largestImage = this.images.find(image => image[0].indexOf(breakpoint.name));
				if (largestImage) {
					return largestImage;
				}
			});
