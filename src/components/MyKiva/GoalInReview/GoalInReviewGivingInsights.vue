<template>
	<section
		class="tw-w-full tw-bg-gray-50 tw-px-2.5 tw-py-5 md:tw-px-4 md:tw-py-7.5"
		data-testid="goal-in-review-giving-insights"
	>
		<p class="tw-text-eco-green-3 tw-mb-1 kv-fade-up giving-insights-eyebrow">
			Giving insights
		</p>

		<h1 class="tw-text-display tw-text-eco-green-4 tw-mb-4 kv-fade-up giving-insights-headline">
			What your goal says
			<span class="tw-block tw-text-marigold">about you</span>
		</h1>

		<ul
			class="tw-grid tw-grid-cols-1 md:tw-grid-cols-3 tw-gap-2
				tw-list-none tw-p-0 tw-m-0"
			data-testid="goal-in-review-giving-insights-cards"
		>
			<li
				class="tw-rounded tw-shadow-lg tw-p-2.5 tw-bg-marigold-1 kv-fade-up giving-insights-card"
				data-testid="goal-in-review-giving-insights-card-origin-story"
			>
				<div class="tw-mb-2">
					<CloudSun
						class="tw-h-5.5 tw-w-5.5"
						viewBox="0 0 50 50"
						aria-hidden="true"
					/>
				</div>

				<p class="tw-text-marigold-3 tw-mb-0.5">
					Your origin story
				</p>
				<h2 class="!tw-text-headline tw-text-eco-green-4 tw-mb-0.5">
					{{ originStory.title }}
				</h2>
				<p class="tw-text-eco-green-4 tw-m-0" v-html="originStory.content"></p>
			</li>

			<li
				class="tw-rounded tw-shadow-lg tw-p-2.5 tw-bg-desert-rose-1 kv-fade-up giving-insights-card"
				data-testid="goal-in-review-giving-insights-card-impact-identity"
			>
				<div class="tw-mb-2">
					<DoubleHandsAndHeart
						class="tw-h-5.5 tw-w-5.5"
						viewBox="0 0 36 40"
						aria-hidden="true"
					/>
				</div>

				<p class="tw-text-desert-rose tw-mb-0.5">
					Your impact identity
				</p>
				<h2 class="!tw-text-headline tw-text-eco-green-4 tw-mb-0.5">
					{{ impactIdentity.title }}
				</h2>
				<p class="tw-text-eco-green-4 tw-m-0" v-html="impactIdentity.content"></p>
			</li>

			<li
				class="tw-rounded tw-shadow-lg tw-p-2.5 tw-bg-eco-green-1 kv-fade-up giving-insights-card"
				data-testid="goal-in-review-giving-insights-card-impact-habit"
			>
				<div class="tw-mb-2">
					<GreenPuzzle
						class="tw-h-5.5 tw-w-5.5"
						viewBox="0 0 42 40"
						aria-hidden="true"
					/>
				</div>

				<p class="tw-text-eco-green-3 tw-mb-0.5">
					Your impact habit
				</p>
				<h2 class="!tw-text-headline tw-text-eco-green-4 tw-mb-0.5">
					{{ impactHabit.title }}
				</h2>
				<p class="tw-text-eco-green-4 tw-m-0" v-html="impactHabit.content"></p>
			</li>
		</ul>
	</section>
</template>

<script setup>
import { computed } from 'vue';
import goalInReviewCopy from '#src/util/goalInReview';
import CloudSun from '#src/assets/images/my-kiva/goal-in-review/cloud-sun.svg';
import DoubleHandsAndHeart from '#src/assets/images/my-kiva/goal-in-review/double-hands-and-heart.svg';
import GreenPuzzle from '#src/assets/images/my-kiva/goal-in-review/green-puzzle.svg';

const props = defineProps({
	goalSummary: {
		type: Object,
		default: null,
	},
	lifetimePercentile: {
		type: Number,
		default: null,
	},
});

const originStory = computed(() => goalInReviewCopy.getOriginStory(props.goalSummary?.dateStarted));
const impactIdentity = computed(() => goalInReviewCopy.getImpactIdentity(props.goalSummary?.category));
const impactHabit = computed(() => goalInReviewCopy.getImpactHabit({
	transactionSessionCount: props.goalSummary?.transactionSessionCount,
	lifetimePercentile: props.lifetimePercentile,
}));
</script>

<style lang="postcss" scoped>
/* Entrance choreography — the shared .kv-fade-up effect lives in
   css/animations.css; this slide sets the rise (24px). The header (eyebrow +
   headline) lands together first, then the three insight cards cascade 100ms
   apart. The modal gates the whole slide until it scrolls into view, so these
   delays count from that moment (see [data-animate-on-view] in the modal). */
.giving-insights-eyebrow,
.giving-insights-headline,
.giving-insights-card {
	--kv-fade-up-distance: 24px;
}

.giving-insights-card:nth-child(1) {
	animation-delay: 0.25s;
}

.giving-insights-card:nth-child(2) {
	animation-delay: 0.35s;
}

.giving-insights-card:nth-child(3) {
	animation-delay: 0.45s;
}
</style>
