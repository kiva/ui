...
let interval = null;
let disconnectRegionWatcher = null;
...
const pendingRegions = computed(() => props.regionsData.filter(region => !region.hasLoans)
...
// --- Methods ---
const regionImageSource = region => {
	const regionImages = {
		Africa,
		Asia,
	...
	delayUntilVisible(() => {
		setTimeout(() => {
			...
			interval = setInterval(() => {
				currentIdx = props.regionsData.findIndex(
					(region, i) => region.hasLoans && !checkedArr.value[i] && i >= currentIdx
				);
				if (currentIdx !== -1) {
...
onBeforeUnmount(() => {
	...
	if (disconnectRegionWatcher) disconnectRegionWatcher();
});
</script>
