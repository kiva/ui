...
<script>
import * as gsap from '../../../assets/js/components/15-years/gsap-core.js';
import * as CustomEase from '../../../assets/js/components/15-years/CustomEase.js';
import * as MorphSVGPlugin from '../../../assets/js/components/15-years/MorphSVGPlugin.js';
...
export default {
    ...
    mounted: function() {
      ...
      const createTimeline = function(id, path, duration) {
        ...
        path.forEach((p, i) => {
          if (i === 1) {
            let p0 = path[0];
            let p1 = p;
            tl = tl.fromTo(id, {morphSVG: {shape: p0.shape, shapeIndex: 0}}, {morphSVG: {shape: p1.shape, shapeIndex: 0, type: (p.type || "linear")}, ease: (p1.ease || standardEase), duration: duration * p1.position});
          }
          else if (i > 1) {
            let p0 = path[i - 1]
            let p1 = p;
            tl = tl.to(id, {morphSVG: {shape: p.shape, shapeIndex: 0, type: (p.type || "linear")}, ease: (p1.ease || standardEase), duration: duration * (p.position - p0.position)});
          }
        })
        return tl;
      }
      ...
      let duration = 240;
      let timelines = this.desktopTimelineDefs.map((p) => {
        let tl = createTimeline("#" + p.id, p.path, duration);
