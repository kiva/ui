	...
	apollo: {
		...
		result({ data }) {
			...
			this.holidayModeEnabled = this.settingEnabled(
				_get(data, 'holiday_enabled.setting.value'),
				_get(data, 'holiday_start_time.setting.value'),
				_get(data, 'holiday_end_time.setting.value')
			);

			this.promoEnabled = this.settingEnabled(
				_get(data, 'promo_enabled.setting.value'),
				_get(data, 'promo_start_time.setting.value'),
				_get(data, 'promo_end_time.setting.value')
			);
		}
	},
	methods: {
