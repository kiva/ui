<template>
	<div class="tw-mb-1">
		...
		<div
			v-if="showKivaCredit"
			:class="`tw-flex tw-flex-row tw-w-full tw-mb-2${ccPageCheck($route) ? '' : ' tw-text-h3' }`"
		...
		>
			<template v-if="showRemoveKivaCredit">
				<div class="tw-w-auto tw-text-left md:tw-text-right tw-flex-1">
					{{ ccPageCheck($route) ? 'Remaining Kiva credit:' : 'Kiva Credit:' }}
				</div>
				<div
				...
				>
					...
					>{{ ccPageCheck($route) ? '' : '- ' }}{{ kivaCredit }}</span>
					<button
		...
		<div v-if="showPromoCreditTotal">
			...
			>
				<span class="tw-w-full tw-text-h3 " v-if="!ccPageCheck($route)">
				...
				<div
					v-if="hasUPCCode && !ccPageCheck($route)"
			...
			<kv-tooltip
				...
				theme="mint"
				v-if="promoFundDisplayDescription && !ccPageCheck($route)"
			>
