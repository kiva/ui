<template>
	...
	>
		...
		<kv-loading-placeholder
			v-if="isLoading"
			class="tw-text-left tw-mb-0.5" :style="{width: 75 + (Math.random() * 15) + '%', height: '1.875rem'}"
		/>

		<borrower-name
			v-if="!isLoading"
			class="tw-text-left tw-mb-0.5 tw-text-h3"
			:max-length="50"
			:name="borrowerName"
		...
		<kv-loading-placeholder
			v-if="isLoading"
			class="tw-text-left tw-mb-0.5" :style="{width: 40 + (Math.random() * 15) + '%', height: '1.3rem'}"
		/>
		...
		/>
		<p v-if="!isLoading" class="tw-text-left tw-mb-2.5 tw-flex-grow">
			{{ loanUse }}
			<kv-text-link
...
import CCLendCta from '@/components/LoanCards/Buttons/CCLendCta';
