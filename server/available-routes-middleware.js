...
/*
 * Extract Routes as formatted for Vue Router, removing async function calls and converting
 * them into a valid json object.
 */
function buildRouteJSON() {
	...
	const routeObject = eval(cleanedRoutes); // eslint-disable-line no-eval
...
try {
	...
	fs.writeFile(
		path.resolve(__dirname, extractedRouteFileName),
		JSON.stringify(availableRoutesJSON),
		err => {
			if (err) {
				console.log(err);
			}
