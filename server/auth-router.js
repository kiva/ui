...
module.exports = function authRouter(config = {}) {
	...
	passport.use(new Auth0Strategy({
		...
		clientID: config.auth0.auth0ServerClientID,
		clientSecret: process.env.UI_AUTH0_CLIENT_SECRET,
		callbackURL: config.auth0.auth0ServerCallbackUri,
	...
	router.get('/ui-logout', (req, res) => {
		...
		const logoutUrl = `https://${config.auth0.auth0Domain}/v2/logout?returnTo=${returnUrl}`;
		req.logout();
